// Build config for the ERPNext Hello applet (ADR-0009). It defers entirely to the OFFICIAL
// Frappe OS preset — the externals (vue/frappe-ui/@frappe-os/api), the stable filename, and
// the dev-harness alias all live there, so an applet author writes only the paths.
//
...
// No `vite` import on purpose: this config resolves its own imports from the erpnext side,
// which ships no node_modules — vite + plugins live in frappe-os (run via its scripts). The
// preset (a frappe-os file) resolves @vitejs/plugin-vue from frappe-os/node_modules; we just
// return the plain config object the preset builds (defineConfig is only a type-hint identity).
...
export default appletConfig({
  ...
  outDir: path.resolve(here, "../../public/os-applets"), // → /assets/erpnext/os-applets/
  fileName: "hello.js",
